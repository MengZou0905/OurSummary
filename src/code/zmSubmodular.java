...
public class zmSubmodular {
    ...
    public int[][] linkOrNot;// add by zm
    ...
    */
    ...
    public void greedy(){
        short []chosen =new short[myDoc.snum];
        //substitued with the primer one.changed by zm
        ...
        while (true){
            ...
            int maxId = -1;
            //need to specify sentences that are  similar to those already selected sentences
            //for example ,sentence j is nearly the same with the chosen sentence i
            // so we make chose[i] = 1, chose[j]=2
            for (int i = 0; i < myDoc.snum; i++){
            ...
            if (maxId == -1) break;
            chosen[maxId] = 1;
            //substitudes with the previous line.changed by zm.
            //chosen[maxId] = true;

            //--------------------add by zm--------------------
            //here need to check the similarity metrix——linkOrNot
            //those who get cosine similarity value larger than the threshold should be marked chose[] as 2
            //namely,if linkOrNot[maxID][j]=1,chose[j] should be set as 2
            for(int i = 0; i < myDoc.snum; ++i){
                if (linkOrNot[maxId][i]==1)
                    chosen[i]=2;
            }
            //-------------------add by zm-----------------------
